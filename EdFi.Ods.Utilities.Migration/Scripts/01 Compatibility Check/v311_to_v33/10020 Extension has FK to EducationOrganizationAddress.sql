...
BEGIN
    PRINT N'Option BypassExtensionValidationCheck enabled. Extension table has foreign key to Education Organization Address check will be skipped.'
END
ELSE
BEGIN
    IF EXISTS
    ...
    WHERE rc.CONSTRAINT_SCHEMA != 'edfi'
        ...
        AND tc.TABLE_NAME='EducationOrganizationAddress'
    )
    BEGIN
        DECLARE @ExtensionFoundErrorMessage NVARCHAR(max) = N'Action Required: Unable to proceed with migration because the BypassExtensionValidationCheck option is disabled, and one or more extension tables were found for Education Organization Address. Please ensure that custom migration scripts have been created for all extension tables, and then enable the --BypassExtensionValidationCheck command line option in order to bypass this check.'
        PRINT @ExtensionFoundErrorMessage;
        THROW 52001, @ExtensionFoundErrorMessage, 1
    END
    ELSE
    BEGIN
        PRINT N'Extension table has foreign key to Education Organization Address check: OK'
    END
END
GO
