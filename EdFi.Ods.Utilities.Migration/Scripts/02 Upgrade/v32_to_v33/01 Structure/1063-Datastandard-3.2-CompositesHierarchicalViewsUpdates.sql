...
IF EXISTS (SELECT *
FROM sys.views
WHERE object_id = OBJECT_ID(N'edfi.LearningStandardH'))
    DROP VIEW edfi.LearningStandardH;
GO

CREATE VIEW edfi.LearningStandardH
AS
    WITH
    LearningStandardH_CTE
    AS
    (
    ...
    FROM edfi.LearningStandard t1
        INNER JOIN LearningStandardH_CTE ON t1.ParentLearningStandardId = LearningStandardH_CTE.LearningStandardId
...
IF EXISTS (SELECT *
FROM sys.views
WHERE object_id = OBJECT_ID(N'edfi.ObjectiveAssessmentH'))
	DROP VIEW edfi.ObjectiveAssessmentH;
GO

CREATE VIEW edfi.ObjectiveAssessmentH
AS
    WITH
    ObjectiveAssessmentH_CTE
    AS
    (
