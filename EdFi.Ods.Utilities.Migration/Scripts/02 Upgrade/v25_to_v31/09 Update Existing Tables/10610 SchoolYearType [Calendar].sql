...
PRINT N'Applying calendar configuration settings to [edfi].[SchoolYearType]'
GO

;WITH [MissingSchoolYears] AS
(
	SELECT DISTINCT m.[SchoolYear]
	FROM [migration_tempdata].[CalendarConfig] m
	LEFT JOIN [edfi].[SchoolYearType] s
	ON m.[SchoolYear] = s.[SchoolYear]
	WHERE s.[SchoolYear] IS NULL
)
